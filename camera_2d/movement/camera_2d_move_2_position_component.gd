		...
		if event is InputEventMouseButton:
			if event.button_index == MOUSE_BUTTON_RIGHT:
				...
				if not event.pressed:
					...
					_target_position_increase = _mouse_click_position - movement_component.get_camera_position()
					movement_component.add_target_position(_target_position_increase)

	...
	if is_ready():
		...
		if _is_dragging:
			if control_mode == 0:
				...
				movement_component.add_target_position(_mouse_delta_viewport_position / movement_component.get_camera_zoom())
				_mouse_prev_viewport_position = get_viewport().get_mouse_position()
			elif control_mode == 1:
				_mouse_current_viewport_position = get_viewport().get_mouse_position()
				movement_component.target_position = _camera_start_position + (_mouse_start_viewport_position - _mouse_current_viewport_position) / movement_component.get_camera_zoom()
	...
	if is_ready():
		...
		if event is InputEventMouseButton:
			if event.button_index == MOUSE_BUTTON_RIGHT:
				...
				# 鼠标右键被按下时
				if event.pressed:
					movement_component.motion_mode = motion_mode # 覆盖摄像机的运动模式
					movement_component.deceleration_speed = deceleration_speed # 覆盖插值移动速度
					movement_component.smoothing_speed = smoothing_speed # 覆盖平滑移动速度
					if control_mode == 0:
						_mouse_prev_viewport_position = get_viewport().get_mouse_position()
					elif control_mode == 1:
						_mouse_start_viewport_position = get_viewport().get_mouse_position() # 记录鼠标在视口中的起始位置
						_camera_start_position = movement_component.get_camera_position() # 记录摄像机的起始位置
